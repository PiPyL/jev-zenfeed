...
(function() {
  ...
  JevFB.hidePost = function(postEl, decision, options = {}) {
    ...
    if (postEl.dataset.jevStatus === 'hidden' && postEl.dataset.jevHideMode === mode) {
      ...
      if (badge) badge.textContent = `Độ tin cậy ${formatConfidence(decision)}%`;
      ...
      if (desc) {
        ...
        desc.textContent = decision && decision.reason
          ...
          : (criteria ? `Khớp tiêu chí: ${truncate(criteria, 70)}` : 'Nội dung khớp tiêu chí lọc của bạn');
        desc.title = criteria;
      }
      return;
    }
    ...
    if (mode === 'blur') {
      ...
      JevFB.injectBlurControls(postEl, decision);
      return;
    }
  ...
  JevFB.injectCollapsedBanner = function(postEl, decision, options) {
    ...
    const detail = reason || (criteria ? `Khớp tiêu chí: ${truncate(criteria, 70)}` : 'Nội dung khớp tiêu chí lọc của bạn');

    banner.innerHTML = `
      <span class="jev-banner-icon">${ICON_SHIELD}</span>
      <div class="jev-banner-info">
        <div class="jev-banner-title">
          <span class="jev-banner-heading">Đã ẩn bài viết</span>
          <span class="jev-badge-conf">Độ tin cậy ${formatConfidence(decision)}%</span>
        </div>
      ...
      <button type="button" class="jev-btn-reveal" aria-expanded="false">
        ${ICON_EYE}<span class="jev-btn-label">Xem</span>
      </button>
    `;
  ...
  /**
   * Toggle post visibility when user clicks 'Xem' / 'Ẩn lại'
   */
  JevFB.toggleReveal = function(postEl, banner) {
    ...
    btn.innerHTML = isRevealed
      ? `${ICON_EYE}<span class="jev-btn-label">Xem</span>`
      : `${ICON_EYE_OFF}<span class="jev-btn-label">Ẩn lại</span>`;
   ...
   */
  JevFB.injectBlurControls = function(postEl, decision) {
    if (postEl.querySelector(':scope > .jev-blur-tag')) return;

    const tag = document.createElement('div');
    tag.className = 'jev-ui jev-blur-tag';
    tag.innerHTML = `
      <span>🛡️ Đã làm mờ (Jev ${formatConfidence(decision)}%)</span>
      <button type="button">Xem nội dung</button>
    `;
